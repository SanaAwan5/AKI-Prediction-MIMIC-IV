...
1. site_C local dominance
   ...
   - site_C retains its full 39-feature set and true MIMIC-IV prevalence
     (12.6%). With richer features than any other site, a locally trained
...
3. Confirmed fixed prevalences (from actual simulation outputs)
   ...
   site_C   9%   Academic anchor (Approach 2 cohort rate)
...
ANCHOR_PREVALENCE = 0.09        # Approach 2 cohort AKI rate (relaxed exclusions)

# Fixed AKI prevalences — clinically grounded lower-end literature values
# site_C fixed to actual Approach 2 cohort rate (9%)
FIXED_PREVALENCES: Dict[str, Optional[float]] = {
    ...
    "site_C": 0.09,    # Academic anchor — actual Approach 2 cohort rate
...
def run_simulation(
    ...
    for site_id, cfg in SITE_CONFIGS.items():
        ...
        site_df = sample_with_prevalence_and_acuity(
            df_source, label_col, target_prev, n=TARGET_N_PER_SITE,
